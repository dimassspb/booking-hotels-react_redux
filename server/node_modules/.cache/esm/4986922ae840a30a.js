let mongoose,bcrypt;_e84‍.x([["default",()=>_e84‍.o]]);_e84‍.w("mongoose",[["default",["mongoose"],function(v){mongoose=v}]]);_e84‍.w("bcrypt",[["default",["bcrypt"],function(v){bcrypt=v}]]);
...
userSchema.pre("save", function (next) {
    ...
    if (user.isModified("password")) {
        return bcrypt.hash(user.password, 12, function (err, hash) {
            if (err) {
                _e84‍.g.console.log("BCRYPT HASH ERR ", err);
                return next(err);
            }
            user.password = hash;
            return next();
...
userSchema.methods.comparePassword = function (password, next) {
    bcrypt.compare(password, this.password, function (err, match) {
        if (err) {
            _e84‍.g.console.log("COMPARE PASSWORD ERR", err);
            return next(err, false);
        }
        // if no err, we get null
        _e84‍.g.console.log("MATCH PASSWORD", match);
        return next(null, match); // true
    });
};

_e84‍.d(mongoose.model("User", userSchema));
